...
(function(vue) {
  ...
  const getLocation = async () => {
    formatAppLog("log", "at utils/location.js:2", "开始获取设备位置");
    return new Promise((resolve, reject) => {
      uni.getLocation({
        type: "wgs84",
        success: async (res) => {
          formatAppLog("log", "at utils/location.js:7", "成功获取设备位置:", res);
          try {
            formatAppLog("log", "at utils/location.js:9", "开始进行高德地图逆地理编码");
            const response = await new Promise((resolve2, reject2) => {
              uni.request({
                url: "https://restapi.amap.com/v3/geocode/regeo",
                data: {
                  location: `${res.longitude},${res.latitude}`,
                  key: "187f81145629c8bf613a9cb906bcda09",
                  extensions: "all"
                },
                success: (res2) => resolve2(res2),
                fail: (err) => reject2(err)
              });
            });
            formatAppLog("log", "at utils/location.js:22", "高德地图逆地理编码成功:", response.data);
            ...
            formatAppLog("error", "at utils/location.js:31", "高德地图逆地理编码失败:", error);
            reject(error);
          }
        },
        fail: (error) => {
          formatAppLog("error", "at utils/location.js:36", "获取设备位置失败:", error);
  ...
  uCharts.prototype.zoom = function() {
    ...
    if (this.opts.enableScroll !== true) {
      formatAppLog("log", "at node_modules/@qiun/ucharts/u-charts.js:7193", "[uCharts] 请启用滚动条后使用");
      return;
    }
  ...
  uCharts.prototype.dobuleZoom = function(e) {
    if (this.opts.enableScroll !== true) {
      formatAppLog("log", "at node_modules/@qiun/ucharts/u-charts.js:7233", "[uCharts] 请启用滚动条后使用");
      return;
    }
  ...
  uCharts.prototype.showToolTip = function(e) {
    ...
    if (!touches) {
      formatAppLog("log", "at node_modules/@qiun/ucharts/u-charts.js:7411", "[uCharts] 未获取到event坐标信息");
    }
    ...
    methods: {
      async getLocation() {
        try {
          ...
          formatAppLog("error", "at pages/index/index.vue:73", "获取位置失败:", error);
        }
      },
        ...
        try {
          ...
          const [weatherRes, aqiRes] = await Promise.all([
            new Promise((resolve, reject) => {
              uni.request({
                url: "https://kv2pg64dv9.re.qweatherapi.com/v7/weather/7d",
                // 修改为7天
                data: {
                  location: `${longitude},${latitude}`,
                  key: "d613d8ea354b449abe9cfe4c6e9bfaff"
                },
            ...
            new Promise((resolve, reject) => {
              uni.request({
                url: "https://kv2pg64dv9.re.qweatherapi.com/v7/air/now",
                data: {
                  location: `${longitude},${latitude}`,
                  key: "d613d8ea354b449abe9cfe4c6e9bfaff"
                },
          ...
          this.weatherData = {
            ...
            so2: aqiRes.now.so2,
            co: aqiRes.now.co,
            dailyData: weatherRes.daily
            // 保存完整的3天数据
          };
          this.drawTempHumidityChart(weatherRes.daily.map((day) => ({
            ...day,
            formattedDate: formatDate(day.fxDate)
            // 添加格式化日期
          })));
        } catch (error) {
          formatAppLog("error", "at pages/index/index.vue:129", "获取天气数据失败:", error);
        }
      },
      drawTempHumidityChart(dailyData) {
        const tempChartData = {
          categories: dailyData.map((item) => item.formattedDate),
          // 使用格式化日期
          series: [
            {
              name: "最高温度",
    ...
    return vue.openBlock(), vue.createElementBlock("view", { class: "container" }, [
      ...
      vue.createElementVNode("view", { class: "header card" }, [
        ...
        vue.createElementVNode(
          "text",
          { class: "location-text" },
          vue.toDisplayString($data.currentLocation),
          1
          /* TEXT */
        )
      ]),
      vue.createCommentVNode(" 天气和空气质量显示部分 "),
      vue.createElementVNode("view", { class: "body" }, [
        $data.weatherData ? (vue.openBlock(), vue.createElementBlock("view", {
          key: 0,
          class: "weather-card card"
        }, [
          vue.createElementVNode("view", { class: "weather-basic" }, [
            vue.createElementVNode(
              "text",
              { class: "weather-text" },
              "天气: " + vue.toDisplayString($data.weatherData.weather),
              1
              /* TEXT */
            ),
            vue.createElementVNode(
              "text",
              { class: "temp-text" },
              "温度: " + vue.toDisplayString($data.weatherData.temperature) + "°C",
              1
              /* TEXT */
            ),
            vue.createElementVNode(
              "text",
              { class: "aqi-text" },
              "空气质量指数: " + vue.toDisplayString($data.weatherData.aqi),
              1
              /* TEXT */
            )
          ]),
          vue.createElementVNode("view", { class: "air-quality-details" }, [
            vue.createElementVNode(
              "text",
              null,
              "PM10: " + vue.toDisplayString($data.weatherData.pm10),
              1
              /* TEXT */
            ),
            vue.createElementVNode(
              "text",
              null,
              "PM2.5: " + vue.toDisplayString($data.weatherData.pm25),
              1
              /* TEXT */
            ),
            vue.createElementVNode(
              "text",
              null,
              "臭氧: " + vue.toDisplayString($data.weatherData.o3),
              1
              /* TEXT */
            ),
            vue.createElementVNode(
              "text",
              null,
              "二氧化氮: " + vue.toDisplayString($data.weatherData.no2),
              1
              /* TEXT */
            ),
            vue.createElementVNode(
              "text",
              null,
              "二氧化硫: " + vue.toDisplayString($data.weatherData.so2),
              1
              /* TEXT */
            ),
            vue.createElementVNode(
              "text",
              null,
              "一氧化碳: " + vue.toDisplayString($data.weatherData.co),
              1
              /* TEXT */
            )
          ])
        ])) : vue.createCommentVNode("v-if", true),
        vue.createCommentVNode(" 温度图表 "),
        vue.createElementVNode("view", { class: "chart-container" }, [
          vue.createElementVNode("view", { class: "chart-title" }, "温度变化 (°C)"),
        ...
        vue.createCommentVNode(" 湿度图表 "),
        vue.createElementVNode("view", { class: "chart-container" }, [
          vue.createElementVNode("view", { class: "chart-title" }, "湿度变化 (%)"),
          vue.createElementVNode("view", { class: "chart card" }, [
            vue.createElementVNode("canvas", {
  ...
  const PagesIndexIndex = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["render", _sfc_render], ["__scopeId", "data-v-1cf27b2a"], ["__file", "E:/20250426/test12/test12/pages/index/index.vue"]]);
  ...
  const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__file", "E:/20250426/test12/test12/App.vue"]]);
  function createApp() {
